...
fetch(`http://${port}/Placementlocal/services/visit/${visitId}`)
...
fetch(`http://${port}/Placementlocal/services/student/display`)
  .then(response => response.json())
  .then(data => {
...
isPlacedError.textContent = '';
    ...
    fetch(`http://${port}/Placementlocal/services/student/create`, {
    method: "POST",
    headers: {
...
function openDialog1() {
  ...
  studentForm.addEventListener('submit', (event) => {
    ...
    fetch(`http://${port}/Placementlocal/services/student/delete/${id}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json'
      },
