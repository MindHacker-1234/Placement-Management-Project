...
fetch(`http://${port}/Placement/services/visit/${visitId}`)
...
fetch(`http://${port}/Placement/services/student/display`)
  .then(response => response.json())
  .then(data => {
...
isPlacedError.textContent = '';
    ...
    fetch(`http://${port}/Placement/services/student/create`, {
    method: "POST",
    headers: {
...
function openDialog1() {
  ...
  studentForm.addEventListener('submit', (event) => {
    ...
    fetch(`http://${port}/Placement/services/student/delete/${id}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json'
      },
