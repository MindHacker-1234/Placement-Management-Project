...
const port = "18.191.127.230:8080";



fetch(`http://${port}/Placement/services/company/display`)
...
form {
  ...
  <label for="placementOfficer">Placement Officer:</label>
    <input type="text" id="placementOfficer" name="placementOfficer"><br><br>
     <span class="error" id="placementOfficerError"></span>
    ...
    <input type="text" id="applicableBranches" name="applicableBranches"><br><br>
      <span class="error" id="applicableBranchesError"></span>

    <label for="companyDescription">Company Description:</label>
    <textarea id="companyDescription" name="companyDescription"></textarea><br><br>
      <span class="error" id="companyDescriptionError"></span>
...
dialog.showModal();
  ...
  companyForm.addEventListener('submit', (event) => {
    ...
    const placementOfficerError = document.getElementById('placementOfficerError');
    const contactError = document.getElementById('contactError');
    const applicableBranchesError = document.getElementById('applicableBranchesError');
    const companyDescriptionError = document.getElementById('companyDescriptionError');

    companyNameError.textContent = '';
    placementOfficerError.textContent = '';
    contactError.textContent = '';
    applicableBranchesError.textContent = '';
    companyDescriptionError.textContent = '';


        // Validate form inputs
        if (companyNameInput === '') {
        companyNameError.textContent='companyName is Required';
        return;
        }
        if (placementOfficerInput === '') {
        placementOfficerError.textContent = 'placementofficername is Required'
        return;
        }
        if (contactInput === '') {
        ...
        if (applicableBranchesInput === '') {
        applicableBranchesError.textContent = 'applicableBranches are Required'
        return;
        }
        if (companyDescriptionInput === '') {
        companyDescriptionError.textContent = 'companyDescription is Required';
        return;
        }
    ...
    fetch(`http://${port}/Placement/services/company/create`, {
        method: 'POST',
