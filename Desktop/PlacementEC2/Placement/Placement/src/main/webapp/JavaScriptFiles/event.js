...
function displayUpcomingVisits() {
    fetch(`http://${port}/Placement/services/visit/display`)
      ...
      .then(data => {
        ...
        upcomingVisits.forEach(visit => {
          ...
          const companyImage = `images/${visit.company.companyName}.png`;
          const visitItem = `
           <div class="card">
                  <div class="company-card">
                  <div class="image-container">
                      <img style="border-radius: 50%; width: 100%; height: 100%" src="${companyImage}" >
                  </div>
                  <div class="company-details">
                      <div class="job-profile-date">
                          <div class="job-profile">- ${jobProfile}</div>
                          <div class="schedule-date">- ${date}</div>
                      </div>
                      <div class="company-name">${companyName} <span class="visitid">${visitId}</span></div>
                      <div class="ctc">${ctc} CTC</div>
    ...
    .then(data => {
      ...
      completedVisits.forEach(visit => {
        ...
        const visitItem = `
                ...
                <div class="company-card">
                <div class="image-container">
                    <img style="border-radius: 50%; height: 150px; width: 150px;" src="${companyImage}" >
                </div>
                <div class="company-details">
                    <div class="job-profile-date">
                        <div class="job-profile">- ${jobProfile}</div>
...
visitList.addEventListener('click', event => {
  ...
  if (target.closest('.card')) {
    const visitId = target.closest('.card').querySelector('.visitid').textContent;
...
return;
  ...
  dialog.remove();
  console.log(data);
  fetch(`http://${port}/Placement/services/visit/create`, {
    method: 'POST',
    headers: {
